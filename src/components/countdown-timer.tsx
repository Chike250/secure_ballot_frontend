...
export function CountdownTimer() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      const targetDate = countdownTarget === 'start' 
  ...
  if (!presidentialElection) {
    return (
      <div className="flex flex-col items-center space-y-4">
        <h3 className="text-lg font-semibold text-foreground">{t("home.countdown.title")}</h3>
        <p className="text-sm text-muted-foreground text-center">No presidential election scheduled</p>
      </div>
    )
  }
    ...
    <div className="flex flex-col items-center space-y-4">
      ...
      <div className="flex items-center justify-center gap-4">
        <div className="flex flex-col items-center text-center">
          ...
          <div className="text-xs text-muted-foreground uppercase tracking-wide">{t("home.countdown.days")}</div>
        </div>
        
        <div className="flex flex-col items-center text-center">
          <div className="text-2xl font-bold text-primary">{timeLeft.hours}</div>
          <div className="text-xs text-muted-foreground uppercase tracking-wide">{t("home.countdown.hours")}</div>
        </div>
        
        <div className="flex flex-col items-center text-center">
          <div className="text-2xl font-bold text-primary">{timeLeft.minutes}</div>
          <div className="text-xs text-muted-foreground uppercase tracking-wide">{t("home.countdown.minutes")}</div>
        </div>
        
        <div className="flex flex-col items-center text-center">
          <div className="text-2xl font-bold text-primary">{timeLeft.seconds}</div>
          <div className="text-xs text-muted-foreground uppercase tracking-wide">{t("home.countdown.seconds")}</div>
        </div>
      </div>
      
      <div className="text-center">
        <p className="text-sm text-muted-foreground">
          {countdownTarget === 'start' ? 'Until voting begins' : 'Until voting ends'}
        </p>
        <p className="text-xs text-muted-foreground mt-1">
          {countdownTarget === 'start' 
            ? `Starts: ${new Date(presidentialElection.startDate).toLocaleDateString()}`
            : `Ends: ${new Date(presidentialElection.endDate).toLocaleDateString()}`
          }
        </p>
      </div>
    </div>
