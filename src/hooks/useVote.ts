...
export function useVote() {
    ...
    async (electionId: string) => {
      ...
      try {
        ...
        const candidatesList = rawCandidatesList.map((candidate: any) => ({
          ...
          color: getPartyColor(
            candidate.partyCode || candidate.partyName || candidate.party
          ),
          bio: candidate.bio,
        ...
        if (status?.data?.hasVoted && status?.data?.isEligible !== undefined) {
          eligibilityData = {
            ...
            reason:
              status.data.eligibilityReason ||
              status.data.reason ||
              (status.data.hasVoted
        ...
        // Otherwise check separate eligibility API response
          ...
          eligibilityData = {
            ...
            reason:
              elig.data.reason ||
              elig.data.eligibilityReason ||
              (elig.data.isEligible
                ? "Eligible to vote"
                : "Not eligible to vote"),
          };
        }
        ...
        // Fallback to direct eligibility object
          ...
          eligibilityData = {
            ...
            reason:
              elig.reason ||
              elig.eligibilityReason ||
              (elig.isEligible ? "Eligible to vote" : "Not eligible to vote"),
          };
        }
        ...
        if (votingStatusData?.hasVoted && votingStatusData.candidateId) {
          setVotedElections((prev) => ({
            ...prev,
            [electionId]: Number(votingStatusData.candidateId),
          }));
        }
      } catch (err: any) {
        setError(
          err.message || `Failed to load election data for ${electionId}`
        );
  ...
  const castVote = useCallback(
    async (
    ...
    ): Promise<{ success: boolean; receiptCode?: string }> => {
      if (!token || !eligibility?.isEligible || votingStatus?.hasVoted) {
        setError(
          eligibility?.reason ||
            (votingStatus?.hasVoted ? "Already voted" : "Cannot cast vote")
        );
      ...
      try {
        ...
        ); // Ensure candidateId is string if API expects it

        // Update local state immediately for responsiveness
        setVotingStatus({ hasVoted: true, candidateId });
        setVotedElections((prev) => ({
          ...prev,
          [electionId]: Number(candidateId),
        }));

        // Update localStorage for profile page consistency (can be removed if profile fetches directly)
        const lsVotingStatus = JSON.parse(
          localStorage.getItem("votingStatus") || "{}"
        );
        const votedCandidateDetails = candidates.find(
          (c) => c.id === candidateId
        );
        ...
        ); // Keep this for dashboard fallback?
