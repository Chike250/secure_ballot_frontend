...
export default function ProfilePage() {
  ...
  const { user: authUser, updateUser, isAuthenticated, token, isInitialized } = useAuthStore()
  ...
  const { elections, hasVoted, votingStatus, checkVotingStatus, fetchElections } = useElectionData()
  ...
  useEffect(() => {
    ...
    console.log('[ProfilePage] UI state:', { isLoading, error, hasProfile: !!profile });
  }, []);

  useEffect(() => {
    console.log('[ProfilePage] Profile state changed:', { profile: !!profile, isLoading });
  }, [profile, isLoading]);
  ...
  useEffect(() => {
    const loadAllData = async () => {
      ...
      if (!isInitialized) {
        console.log('[ProfilePage] Auth not initialized yet, waiting...');
        return;
      }

      if (!isAuthenticated || !token) {
        console.log('[ProfilePage] Not authenticated or no token, skipping data load');
      ...
      try {
        ...
        console.log('[ProfilePage] Data load completed');
      } catch (error) {
  ...
  useEffect(() => {
    const initialData = {
      ...
      pollingUnit: profile?.voterCard?.pollingUnitCode || authUser?.pollingUnitCode || "",
      bio: profile?.bio || "",
    };
    ...
    if (elections.length > 0) {
      const loadVotingStatuses = async () => {
        try {
          ...
          if (error.message?.includes('TOO_MANY_REQUESTS') || error.code === 'TOO_MANY_REQUESTS') {
            setError('Rate limit reached. Please refresh the page to try again.');
          }
        }
      };
  ...
  return (
    <div className="container mx-auto py-6 px-4 max-w-5xl">
      ...
      {error && (
        <Alert variant={error.includes('Rate limit') ? "default" : "destructive"} className="mb-4">
          <AlertCircle className="h-4 w-4" />
          <AlertTitle>{error.includes('Rate limit') ? "Rate Limit Notice" : "Error"}</AlertTitle>
          <AlertDescription>
            {error}
        ...
        <div className="md:w-1/3">
          ...
          <Card className="mt-6">
            <CardHeader>
              ...
              <CardDescription>Your voting history across all elections</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {isLoading ? (
                  ...
                  elections.map((election) => (
                    <div key={election.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex-1">
                        <h3 className="text-sm font-medium mb-1">{election.name}</h3>
                        <p className="text-xs text-muted-foreground mb-2">
                          {election.type} • {new Date(election.startDate).toLocaleDateString()} - {new Date(election.endDate).toLocaleDateString()}
                        </p>
                        {hasVoted[election.id] ? (
                          <Badge className="bg-green-500">
                            ...
                            {votingStatus[election.id]?.candidateName && (
                              <span> for {votingStatus[election.id].candidateName}</span>
                            )}
                            {votingStatus[election.id]?.candidateParty && (
                              <span> ({votingStatus[election.id].candidateParty})</span>
                            )}
                          </Badge>
                        ) : votingStatus[election.id] === undefined && election.status === 'active' ? (
                          ...
                          <Badge variant="outline">
                            {election.status === 'scheduled' ? 'Upcoming' : 'Not Available'}
                          </Badge>
                        )}
                      </div>
                      {election.status === 'active' && !hasVoted[election.id] && (
                        <Button size="sm" asChild>
                          <Link href={`/vote?election=${election.id}`}>Vote Now</Link>
                        </Button>
                      )}
                    </div>
                  ))
                  ...
                  <div className="text-center py-8">
                    <p className="text-sm text-muted-foreground mb-2">No elections available</p>
                    <p className="text-xs text-muted-foreground">Check back later for upcoming elections</p>
                  </div>
                )}
              </div>
            </CardContent>
              ...
              {profile?.voterCard?.pollingUnit ? (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <p><span className="font-medium">Name:</span> {profile.voterCard.pollingUnit.name}</p>
                    <p><span className="font-medium">Code:</span> {profile.voterCard.pollingUnit.code}</p>
                    {profile.voterCard.pollingUnit.address && (
                      <p><span className="font-medium">Address:</span> {profile.voterCard.pollingUnit.address}</p>
                    )}
                    <p><span className="font-medium">VIN:</span> {profile.voterCard.vin}</p>
                  </div>
                  ...
                  {showDetailedPollingInfo && detailedPollingUnit && (
                    <>
                      <Separator />
                      <div className="space-y-2 text-sm">
                        <h4 className="font-medium text-base mb-3">Detailed Information</h4>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          <p><span className="font-medium">State:</span> {detailedPollingUnit.state}</p>
                          <p><span className="font-medium">LGA:</span> {detailedPollingUnit.lga}</p>
                          <p><span className="font-medium">Ward:</span> {detailedPollingUnit.ward}</p>
                          <p><span className="font-medium">Registered Voters:</span> {detailedPollingUnit.registeredVoters?.toLocaleString()}</p>
                        </div>
                        
                        {detailedPollingUnit.assignedOfficer && (
                          <p><span className="font-medium">Assigned Officer:</span> {detailedPollingUnit.assignedOfficer}</p>
                        )}
                        
                        {(detailedPollingUnit.latitude && detailedPollingUnit.longitude) && (
                          <p><span className="font-medium">Coordinates:</span> {detailedPollingUnit.latitude}, {detailedPollingUnit.longitude}</p>
                        )}
                        
                        <div className="flex items-center space-x-2 mt-2">
                          <Badge variant={detailedPollingUnit.isActive ? "default" : "destructive"}>
                            {detailedPollingUnit.isActive ? "Active" : "Inactive"}
                          </Badge>
                        </div>
                      </div>
                    </>
                ...
                <div className="space-y-2 text-sm">
                  <p><span className="font-medium">VIN:</span> {profile.voterCard.vin}</p>
                  <p><span className="font-medium">Polling Unit Code:</span> {profile.voterCard.pollingUnitCode}</p>
                  <Alert className="mt-4">
                    <AlertCircle className="h-4 w-4" />
                    <AlertTitle>Polling Unit Details</AlertTitle>
                    <AlertDescription>
                      Detailed polling unit information is not available. Please contact the electoral commission if you need assistance.
                    </AlertDescription>
                  </Alert>
                </div>
              ) : (
                <div className="text-center py-4">
                  <p className="text-sm text-muted-foreground">No voter card information available</p>
                </div>
              )}
            </CardContent>
          </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="vin">Voter Identification Number (VIN)</Label>
                      <Input
                        id="vin"
                        value={formData.vin}
                        disabled
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      Your voter registration details are verified and cannot be modified. 
                      If you need to update this information, please contact the electoral commission.
                    </AlertDescription>
                  </Alert>
                </CardContent>
              </Card>
